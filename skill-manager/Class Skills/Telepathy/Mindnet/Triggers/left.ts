...
export const left = new TriggerItem(
    'Left',
    /^([A-Z][a-z]+) has left the area\.$/,
            ...
            function (args: TriggerFunctionArgs & { 1: string }) {
                if (client.skillManager.skills.class.pranks.active) {
                    ...
                    if (person && client.peopleManager.isCityEnemy(person)) {
                        client.systemService.sendCommand(`pt Sensed City Enemy ${client.peopleManager.displayify(person.name)} has left '${area}'`);

                        if (client.peopleManager.you && client.peopleManager.you.city && client.peopleManager.you.city.toLowerCase() === area.toLowerCase()) {
                            client.systemService.sendCommand(`armytell Sensed City Enemy ${client.peopleManager.displayify(person.name)} has left '${area}'`);
                        }
                    }
                }
            }

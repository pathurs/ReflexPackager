...
export const person = new TriggerItem(
    'Person',
    /^You sense ([A-Z][a-z]+) at ([\w\W]+)\.$/,
            ...
            function (args: TriggerFunctionArgs & { 1: string; 2: string }) {
                if (client.skillManager.skills.class.pranks.active) {
                    ...
                    if (person && client.peopleManager.isCityEnemy(person)) {
                        client.systemService.sendCommand(`pt Sensed City Enemy ${client.peopleManager.displayify(person.name)} at '${location}' in '${area}'`);
